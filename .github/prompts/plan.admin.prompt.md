...
課題の実施方式は以下の4つから選択します。詳細は [ASSIGNMENT_TYPES.md](./ASSIGNMENT_TYPES.md) を参照してください。

- **プログラム実装課題**: 白紙またはスケルトンコードから実装し、REDテストをGREENにしていく
- **リファクタリング課題**: 既存コードをGREENテストを維持しながら品質改善
- **テスト実装課題**: バグのあるコードに対してテストを記述してバグを検出
- **テスト駆動開発課題**: RED→GREEN→リファクタリングのサイクルで実装